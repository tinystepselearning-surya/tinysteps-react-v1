...
export const MAX_CHILD_PROGRESS_REBUILD_DOCS = 250;
...
/**
 * Canonical curriculum completion rule.
 *
 * The teacher editor writes one progress document per canonical lesson only when Save,
 * Save & Back, or Save & Next succeeds. Therefore an existing canonical progress document
 * is the durable receipt that the lesson was taught and saved. Re-saving the same lesson
 * updates the same document and must never add another completion.
 *
 * Skill stars, mastery, attendance, and the optional learning-status choice may describe
 * how confidently the child performed, but none of them change the one-completion-per-saved-
 * lesson rule.
 */
...
export function projectionMatchesCurriculum(
  ...
  if (!existing || existing.schemaVersion !== 3 || existing.modelType !== 'child_course_progress_v3') return false;
  if (existing.completionAuthority !== 'teacher_progress_save') return false;
  if (normalizeCourseId(existing.courseId) !== normalizeCourseId(courseId)) return false;
  if (Number(existing.inProgressTopics ?? 0) !== 0) return false;
...
function normalizeStage(stage: StageProgressSummary): StageProgressSummary {
  ...
  const inProgress = 0;
  const notStarted = Math.max(0, total - completed);
  return {
    ...stage,
    totalTopics: total,
...
function finalizeSummary(args: {
  ...
  const inProgressTopics = 0;
  const notStartedTopics = Math.max(0, totalTopics - completedTopics);
...
export function applyIncrementalSummary(args: {
  ...
  const adjust = (topic: CurriculumTopic | null, state: ProgressState, direction: -1 | 1) => {
    if (!topic) return;
    const stage = byKey.get(stageKey(topic.stageOrder, topic.stageLabel));
    if (!stage) return;
...
async function boundedProgressDocs(
...
async function updateCourseSummary(args: {
  ...
  beforeData: ProgressData;
  afterData: ProgressData;
}): Promise<{ sourceDocumentsRead: number; mode: 'incremental_v3' | 'bootstrap_v3' | 'duplicate' }> {
  ...
  const preflight = await Promise.all([eventRef.get(), summaryRef.get(), curriculumRef.get()]);
  const [eventSnap, currentSnap, curriculumSnap] = preflight;
  if (eventSnap.exists) return { sourceDocumentsRead: 1, mode: 'duplicate' };
  ...
  if (projectionMatchesCurriculum(currentSnap.exists ? currentSnap.data() as Partial<CourseProgressSummary> : null, courseId, topics)) {
    ...
    batch.set(summaryRef, next, { merge: false });
    batch.set(eventRef, {
      schemaVersion: 3,
      projectionAppliedAt: FieldValue.serverTimestamp(),
    });
    await batch.commit();
    return { sourceDocumentsRead: 3, mode: 'incremental_v3' };
  }

  const progressSnap = await boundedProgressDocs(db, kidId);
  ...
  const batch = db.batch();
  batch.set(summaryRef, bootstrap, { merge: false });
  batch.set(eventRef, {
    schemaVersion: 3,
    projectionAppliedAt: FieldValue.serverTimestamp(),
  });
 ...
 * The exported function name intentionally stays unchanged so deployment replaces the P3
 * trigger rather than installing a second writer.
 */
  ...
  async (event) => {
    ...
    if (beforeCourseId && afterCourseId && beforeCourseId === afterCourseId) {
      ...
      if (beforeCourseId) {
        ...
        modes.push('rebuild_previous_course_v3');
      }
      if (afterCourseId) {
        const result = await updateCourseSummary({
          db,
