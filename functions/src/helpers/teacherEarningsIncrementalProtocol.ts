...
export type TeacherEarningsPreplannedIncrementalDelta = {
  target: TeacherMonthRollupTarget;
  delta: TeacherEarningsContribution;
};
...
const COUNT_FIELDS: Array<keyof TeacherEarningsIncrementalTotals> = [
  'totalSessions', 'sessionsCompleted', 'demoCompletedCount', 'demoEnrollmentBonusCount',
];
const TOTAL_FIELDS: Array<keyof TeacherEarningsIncrementalTotals> = [
  'totalEarnings', 'pendingEarnings', 'totalSessions', 'sessionsCompleted', 'demoEarnings',
  'demoCompletedCount', 'demoEnrollmentBonusCount',
];
...
const nonNegativeInteger = (value: unknown): number | null => {
  ...
  return parsed == null || parsed < 0 || !Number.isInteger(parsed) ? null : parsed;
};
const timestampMillis = (value: unknown): number | null => {
  ...
  if (typeof value === 'object') {
    const row = value as { toMillis?: () => number; seconds?: number; nanoseconds?: number; _seconds?: number; _nanoseconds?: number };
...
const stableValue = (value: unknown): unknown => {
  ...
  if (typeof value === 'object') {
    const row = value as Record<string, unknown> & { toMillis?: () => number; seconds?: number; nanoseconds?: number; _seconds?: number; _nanoseconds?: number };
    if (typeof row.toMillis === 'function') {
      const millis = row.toMillis();
    ...
    const seconds = finiteNumber(row.seconds ?? row._seconds);
    const nanoseconds = finiteNumber(row.nanoseconds ?? row._nanoseconds ?? 0);
    if (seconds != null) return { __timestampSeconds: seconds, __timestampNanoseconds: nanoseconds ?? 0 };
...
export const teacherEarningsIncrementalMarkerId = (eventId: unknown): string | null => {
  ...
  return normalized ? `b7_${sha256(normalized).slice(0, 40)}` : null;
};
export const teacherEarningsIncrementalChangeSignature = (input: {
  eventId: unknown; earningId: unknown; before: Record<string, unknown> | null; after: Record<string, unknown> | null;
}): string | null => {
  const eventId = normalizeText(input.eventId);
  const earningId = normalizeText(input.earningId);
  if (!eventId || !earningId) return null;
  return sha256(JSON.stringify(stableValue({ eventId, earningId, before: input.before, after: input.after })));
};
const rollupMatchesTarget = (rollup: Record<string, unknown>, target: TeacherMonthRollupTarget): boolean => {
  const month = normalizeText(rollup.monthKey || rollup.month);
  return Boolean(month && month === target.monthKey);
};
const readRollupTotals = (rollup: Record<string, unknown>): TeacherEarningsIncrementalTotals | null => {
  ...
  for (const field of COUNT_FIELDS) if (!Number.isInteger(out[field])) return null;
  return out;
};
const addDelta = (totals: TeacherEarningsIncrementalTotals, delta: TeacherEarningsContribution): TeacherEarningsIncrementalTotals | null => {
  ...
  for (const field of COUNT_FIELDS) if (!Number.isInteger(next[field]) || next[field] < 0) return null;
...
export const planTeacherEarningsIncrementalTransaction = (input: {
  ...
  preplannedDelta?: TeacherEarningsPreplannedIncrementalDelta | null;
  ...
  const planned = input.preplannedDelta
    ? { mode: 'delta' as const, target: input.preplannedDelta.target, delta: input.preplannedDelta.delta }
    : planTeacherEarningsRollupChange({ earningId: normalizeText(input.earningId), before: input.before, after: input.after });
  if (planned.mode !== 'delta') return { mode: 'fallback', reason: planned.mode === 'recompute' ? `planner_${planned.reason}` : 'planner_noop' };
  ...
  if (source !== 'teacherEarnings_events_v1' || rollupVersion == null || rollupVersion < 1) return { mode: 'fallback', reason: 'rollup_not_authoritative_v1' };
  if (!rollupMatchesTarget(rollup, planned.target)) return { mode: 'fallback', reason: 'rollup_target_mismatch' };
  if (protocolVersion == null || protocolVersion < TEACHER_EARNINGS_INCREMENTAL_PROTOCOL_VERSION || transactionFence !== TEACHER_EARNINGS_TRANSACTION_FENCE || revisionBefore == null) return { mode: 'fallback', reason: 'recompute_not_transaction_coordinated' };
  if (recomputeState !== 'idle') return { mode: 'fallback', reason: 'recompute_in_progress' };
  ...
  if (authoritativeCommittedAt == null || eventUpdateAt == null) return { mode: 'fallback', reason: 'authoritative_watermark_missing' };
  if (eventUpdateAt < authoritativeCommittedAt) return { mode: 'covered', reason: 'event_already_in_authoritative_baseline' };
  if (eventUpdateAt === authoritativeCommittedAt) return { mode: 'fallback', reason: 'authoritative_watermark_ambiguous' };
  const currentTotals = readRollupTotals(rollup);
  if (!currentTotals) return { mode: 'fallback', reason: 'rollup_totals_invalid' };
  const nextTotals = addDelta(currentTotals, planned.delta);
  if (!nextTotals) return { mode: 'fallback', reason: 'delta_would_violate_rollup_invariants' };
  return { mode: 'candidate', target: planned.target, markerId, changeSignature, protocolVersion: TEACHER_EARNINGS_INCREMENTAL_PROTOCOL_VERSION, revisionBefore, revisionAfter: revisionBefore + 1, delta: planned.delta, nextTotals };
};

export const evaluateTeacherEarningsIncrementalReplay = (input: {
  markerId: string; changeSignature: string; existingMarker: Record<string, unknown> | null;
  ...
  if (existingSignature === input.changeSignature && existingProtocolVersion != null && existingProtocolVersion >= TEACHER_EARNINGS_INCREMENTAL_PROTOCOL_VERSION) return { mode: 'replay', markerId: input.markerId };
