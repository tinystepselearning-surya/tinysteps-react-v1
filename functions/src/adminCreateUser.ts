...
export const adminCreateUser = onCall({
  ...
  if (!data.email || !data.password || !data.firstName || !data.lastName || !data.username || !data.role) {
    throw new HttpsError('invalid-argument', 'Missing required fields: email, password, firstName, lastName, username, role');
  }
  ...
  if (data.role === 'student' && !data.parentId) {
    throw new HttpsError('invalid-argument', 'Students must have a parent assigned');
  }
