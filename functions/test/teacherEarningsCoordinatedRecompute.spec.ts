...
describe('B6 Brick 7B2 coordinated authoritative recompute', () => {
  ...
  it('preserves legacy timestamp-derived month targeting when monthKey is absent', () => {
    expect(
      resolveTeacherEarningsRecomputeTarget({
        teacherId: 'teacher-1',
  ...
  it('keeps the coordinated scanner month-bounded and never writes source ledgers', () => {
    ...
    expect(source).toContain('planTeacherEarningsRecomputeClaim');
    expect(source).toContain('planTeacherEarningsRecomputeFinalize');
