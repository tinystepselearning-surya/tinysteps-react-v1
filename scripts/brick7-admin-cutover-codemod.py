...
# Remove schedule-stats-only import; rolling admin UI no longer scans all sessions.
s = s.replace("import { doesSessionMatchEnrollmentSchedule } from '../../../lib/sessionScheduleIntegrity';\n", '')

# Extend the enrollment schedule type with the canonical rolling contract while retaining legacy read fields.
old = """  schedule?: {
    timezone?: string;
...
"""
if old not in s:
    raise SystemExit('Enrollment schedule type anchor missing')
s = s.replace(old, new, 1)

# Remove the obsolete finite coverage estimator and full-history modal statistics helpers.
pattern = re.compile(r"const SCHEDULE_EXCEPTION_SOURCE_TOKENS = \[.*?\nfunction formatYMDCompact\(d: Date\): string \{", re.S)
m = pattern.search(s)
if not m:
    raise SystemExit('finite coverage/stat helper block missing')
s = s[:m.start()] + "function formatYMDCompact(d: Date): string {" + s[m.end():]

# Replace finite schedule state with one rolling lifecycle action state.
old = """  const [feePerClass, setFeePerClass] = useState<number>(0);
...
"""
new = """  const [feePerClass, setFeePerClass] = useState<number>(0);
...
"""
if old not in s:
    raise SystemExit('schedule state block missing')
s = s.replace(old, new, 1)

# Remove finite form hydration and coverage preview.
old = """    setDurationMins(normalizedSlots[0]?.durationMinutes ?? baseDuration);
...
"""
new = """    setDurationMins(normalizedSlots[0]?.durationMinutes ?? baseDuration);
    setFeePerClass(safeNumber(enrollment.feePerClass, 0));
"""
if old not in s:
    raise SystemExit('finite hydration block missing')
s = s.replace(old, new, 1)
preview = re.compile(r"  const plannedCoveragePreview = useMemo\(.*?\n  \);\n", re.S)
s, count = preview.subn('', s, count=1)
if count != 1:
    raise SystemExit('plannedCoveragePreview block missing')

# Add a local rolling-contract detector used for safe gradual conversion.
anchor = "function normalizeEnrollmentStatus(value: any): string {\n"
helper = """function isCanonicalRollingEnrollmentForAdmin(enrollment: EnrollmentLite | null | undefined): boolean {
  return Boolean(
    enrollment?.schedule &&
...
"""
if anchor not in s:
    raise SystemExit('normalizeEnrollmentStatus anchor missing')
s = s.replace(anchor, helper + anchor, 1)

# Remove the broad classSessions-by-enrollment modal statistics effect and finite progress display values.
pattern = re.compile(
...
m = pattern.search(s)
if not m:
    raise SystemExit('schedule live stats/progress block missing')
replacement = """  const selectedScheduleStatus = normalizeEnrollmentStatus(selectedScheduleEnrollment?.status);
...
# Remove finite resets on modal open.
old = """    setScheduleEnrollmentId(first.id);
    setGenerateWeeks(8);
    setEndDate('');
    setPauseUpcomingCount(1);
    applyScheduleFormFromEnrollment(first);
"""
new = """    setScheduleEnrollmentId(first.id);
    applyScheduleFormFromEnrollment(first);
"""
if old not in s:
    raise SystemExit('open modal finite reset block missing')
s = s.replace(old, new, 1)

# Replace legacy finite schedule save/pause/resume orchestration with rolling activation/reconciliation/lifecycle.
start = s.index('  async function handleSaveSchedule() {')
end = s.index('  async function handleCreateAdHocSession()', start)
new_handlers = r'''  async function handleSaveSchedule() {
...
# Replace finite-control section of the modal with rolling lifecycle controls.
start_marker = '                <div>\n                  <div className="text-sm font-medium mb-1">Generate for (weeks)</div>'
end_marker = '                <div className="md:col-span-2">\n                  <div className="text-sm font-medium mb-1">Teams meeting link (optional)</div>'
start = s.index(start_marker)
end = s.index(end_marker, start)
rolling_ui = r'''                <div className="md:col-span-2 rounded-lg border border-slate-200 bg-slate-50 p-3">
...
'''
s = s[:start] + rolling_ui + s[end:]

# Update modal copy to reflect permanent recurrence.
s = s.replace(
...
# Disable save for terminal enrollments; legacy non-terminal enrollments can be converted on first save.
old = """            <Button onClick={handleSaveSchedule} disabled={savingSchedule}>
              {savingSchedule ? 'Saving...' : 'Save Schedule'}
            </Button>"""
new = """            <Button onClick={handleSaveSchedule} disabled={savingSchedule || scheduleLifecycleAction !== null || selectedScheduleIsTerminal}>
              {savingSchedule ? 'Saving...' : selectedScheduleIsRolling ? 'Save Schedule' : 'Activate Continuous Schedule'}
            </Button>"""
if old not in s:
    raise SystemExit('Save Schedule button anchor missing')
s = s.replace(old, new, 1)

if s == original:
    raise SystemExit('codemod made no changes')
...
forbidden = [
    ...
    'Pause upcoming classes',
    'End date (optional)',
    'setGenerateWeeks(',
    'setPlannedSessions(',
    'setPauseUpcomingCount(',
    'scheduleLiveStats',
    'plannedCoveragePreview',
]
remaining = [token for token in forbidden if token in s]
if remaining:
    raise SystemExit(f'obsolete schedule tokens remain: {remaining}')
for required in ['saveRollingEnrollmentSchedule', 'reconcileRollingEnrollmentSchedule', 'setRollingEnrollmentLifecycle']:
    if required not in s:
        raise SystemExit(f'missing rolling callable: {required}')
