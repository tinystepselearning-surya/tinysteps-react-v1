...
import {
  ...
  ensureRequiredIndex,
  isRequiredIndex,
} from './ensure-leads-status-created-at-index.mjs';

const requiredIndex = (state = 'READY') => ({
  name: 'projects/test/databases/(default)/collectionGroups/leads/indexes/required',
...
test('READY status+createdAt index succeeds without mutation', async () => {
...
test('missing status+createdAt index is created once and polled until READY', async () => {
  ...
  assert.ok(createArgs.includes('--field-config=field-path=status,order=ascending'));
  assert.ok(createArgs.includes('--field-config=field-path=createdAt,order=descending'));
...
test('similar status+createdAt+source index is not treated as required', () => {
  const extraField = requiredIndex();
  extraField.fields.splice(2, 0, { fieldPath: 'source', order: 'ASCENDING' });
  assert.equal(isRequiredIndex(extraField), false);
});

test('wrong createdAt direction is rejected', () => {
  const wrong = requiredIndex();
  wrong.fields[1].order = 'ASCENDING';
