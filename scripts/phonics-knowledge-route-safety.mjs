...
export function validateR8ChangedPaths(paths, { allowApprovedR9 = false } = {}) {
  return paths.filter((p) => !R8_CHANGE_PATH.test(p) && !(allowApprovedR9 && R9_CHANGE_PATH.test(p))).map((p) => ({ code: 'public-surface-delta', id: p, detail: 'Brick 8 may change only its own contract unless the path is part of the explicit approved Brick 9 publication or R9.1 governance surface.' }));
}
