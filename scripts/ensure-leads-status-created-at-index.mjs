...
const displayNameOf = (index) =>
  String(index?.name || `${collectionGroup}(status ASC, createdAt DESC)`);
...
export const ensureRequiredIndex = async ({
  ...
  let created = false;
  if (!initial) {
    log(`Creating required Firestore index ${collectionGroup}(status ASC, createdAt DESC)...`);
    try {
      runGcloud([
        'firestore',
        'indexes',
  ...
  while (true) {
    if (current) {
      ...
      log(
        `Waiting for Firestore index ${collectionGroup}(status ASC, createdAt DESC): ${state || 'BUILDING'}...`,
      );
    } else {
      log(`Waiting for Firestore index ${collectionGroup}(status ASC, createdAt DESC) to appear...`);
    }
  ...
  throw new Error(
    `Timed out waiting for Firestore index ${collectionGroup}(status ASC, createdAt DESC) to become READY`,
  );
};
...
if (isCli) {
  ...
  if (!project) {
    console.error('Usage: node scripts/ensure-leads-status-created-at-index.mjs --project <project-id>');
    process.exitCode = 64;
  } else {
    try {
