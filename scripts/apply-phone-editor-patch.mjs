...
const file = 'src/pages/admin/TodaysNotifications.tsx';
...
replaceOnce(
  ...
  'structured phone display/edit value',
);
...
replaceOnce(
  ...
  'phone editor start handler',
);

const savePattern = /  const handleSavePhoneEdit = async \(\) => \{[\s\S]*?\n  \};\n\n  const isOverallAdmissionsMode/;
if (!savePattern.test(source)) throw new Error('Patch anchor not found: phone save handler');
...
const rawPhoneSetter = `prev ? { ...prev, phone: event.target.value } : prev`;
const numericPhoneSetter = `prev ? { ...prev, phone: digitsOnly(event.target.value) } : prev`;
const setterOccurrences = source.split(rawPhoneSetter).length - 1;
if (setterOccurrences !== 2) throw new Error(`Expected two inline phone inputs, found ${setterOccurrences}`);
source = source.split(rawPhoneSetter).join(numericPhoneSetter);

const phoneInputAnchor = `                                    placeholder="Phone number"\n                                    className="h-8 text-xs"`;
const phoneInputReplacement = `                                    type="tel"\n                                    inputMode="numeric"\n                                    pattern="[0-9]*"\n                                    placeholder="Phone number"\n                                    className="h-8 text-xs"`;
const inputOccurrences = source.split(phoneInputAnchor).length - 1;
if (inputOccurrences !== 2) throw new Error(`Expected two inline phone input anchors, found ${inputOccurrences}`);
source = source.split(phoneInputAnchor).join(phoneInputReplacement);
