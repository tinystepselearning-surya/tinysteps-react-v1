...
const displayNameOf = (index) =>
  String(index?.name || `${collectionGroup}(parentId ASC, date ASC)`);
...
export const ensureRequiredIndex = async ({
  ...
  let created = false;
  if (!initial) {
    log(`Creating required Firestore index ${collectionGroup}(parentId ASC, date ASC)...`);
    try {
      runGcloud([
        'firestore',
        'indexes',
  ...
  while (true) {
    if (current) {
      ...
      log(
        `Waiting for Firestore index ${collectionGroup}(parentId ASC, date ASC): ${state || 'BUILDING'}...`,
      );
    } else {
      log(`Waiting for Firestore index ${collectionGroup}(parentId ASC, date ASC) to appear...`);
    }
  ...
  throw new Error(
    `Timed out waiting for Firestore index ${collectionGroup}(parentId ASC, date ASC) to become READY`,
  );
};
