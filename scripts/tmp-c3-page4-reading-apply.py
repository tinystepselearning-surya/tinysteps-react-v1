...
page_path = Path("src/pages/public/ReadingClassesForKidsPage.tsx")
...
page = replace_once(
    ...
    "",
    "remove phonics cluster nav import",
)

old_keywords = """const READING_SEO_KEYWORDS = [
...
new_keywords = """const READING_SEO_KEYWORDS = [
  'online reading classes for kids',
  'reading classes for kids',
...
page = replace_once(page, old_keywords, new_keywords, "reading keyword ownership")
...
page = replace_once(
    ...
    "hero search intent and routing",
)
...
page = replace_once(
    ...
    "quick answer fluency ownership",
)
...
page = replace_once(
    ...
    "standard class duration wording",
)

fluency_stage_anchor = """          <div className=\"mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3\">
...
fluency_stage_new = fluency_stage_anchor + """
...
page = replace_once(page, fluency_stage_anchor, fluency_stage_new, "visible fluency owner handoff")
...
page = replace_once(
  ...
  {
    question: 'How does Tiny Steps decide the right reading path?',""",
    "routing and international FAQs",
)
...
page = replace_once(
    ...
    "",
    "remove wrong Explore Phonics footer",
)
...
old_registry = """  '/reading-classes-for-kids': {
...
new_registry = """  '/reading-classes-for-kids': {
...
registry = replace_once(registry, old_registry, new_registry, "prerender reading metadata")
registry_path.write_text(registry)

graph_path = Path("src/lib/structuredDataGraph.ts")
graph = graph_path.read_text()
old_service = """    name: 'Tiny Steps Reading Support Programme',
    description:
...
new_service = """    name: 'Tiny Steps Online Reading Classes for Kids',
    description:
...
graph = replace_once(graph, old_service, new_service, "reading Service schema")
