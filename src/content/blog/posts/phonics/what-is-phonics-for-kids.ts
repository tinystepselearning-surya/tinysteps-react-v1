...
const post: BlogPost = {
  ...
  body: [
    ...
    { type: 'p', content: 'A grapheme is a written representation of a phoneme. The link between a grapheme and a phoneme is often called a **grapheme–phoneme correspondence (GPC)**. A grapheme can be one letter, such as **m**, or more than one letter, such as **sh**, **ai** or **igh**. English has multiple spellings for some sounds and some graphemes can represent more than one sound, which is why phonics needs a planned cumulative sequence rather than one slogan per vowel.' },
    ...
    { type: 'p', content: 'For spelling **ship**, the child hears the word, identifies /sh/ /i/ /p/ and represents those phonemes with the graphemes **sh-i-p**. This sound-to-print direction is often called **encoding**; reading a printed word through its graphemes is **decoding**. Blog #9, [How Phonics Improves Spelling](/blog/how-phonics-improves-spelling), owns the full encoding roadmap.' },
    ...
    { type: 'h2', content: 'Phonemic awareness and phonics are related, but they are not the same' },
    { type: 'p', content: '**Phonemic awareness** means hearing and working with individual speech sounds without needing to see print. **Phonics** begins when those speech sounds are connected with written graphemes. A child might be able to hear the first sound in **sun** orally but still need teaching to connect /s/ with the printed letter **s**.' },
    { type: 'p', content: 'The skills support each other, but they should not be treated as interchangeable labels. For the full distinction between phonological awareness, phonemic awareness and phonics, use [Phonological Awareness vs Phonemic Awareness vs Phonics](/blog/phonological-awareness-vs-phonemic-awareness-vs-phonics).' },
    ...
    { type: 'h2', content: 'What phonics is — and what it is not' },
    { type: 'h3', content: 'Phonics is not the same as learning the alphabet' },
    { type: 'p', content: 'Alphabet knowledge is useful, but knowing letter names does not automatically teach a child how to read an unfamiliar word. A child can sing A–Z, point to **B**, call it “bee,” and still not know how **b** contributes /b/ when decoding **bat**. That gap is why “knows ABC” and “can decode” should be checked separately. See [Child Knows ABC but Cannot Read](/blog/child-knows-abc-but-cannot-read) for the full parent diagnostic boundary.' },
    { type: 'h3', content: 'Phonics is not whole-word shape memorisation' },
    { type: 'p', content: 'Children do build a store of rapidly recognised words as reading develops, but that does not require teaching every common word as an unanalyzable visual shape. Many high-frequency words are fully or largely decodable once the relevant correspondences are known. When a common word contains an unusual or not-yet-taught part, useful teaching maps the regular parts and draws attention to the unexpected part. See [Phonics vs Sight Words](/blog/science-of-phonics-learning) and [How to Teach Tricky Words](/blog/phonics-tricky-words).' },
    { type: 'h3', content: 'Phonics is not picture or context guessing' },
    { type: 'p', content: 'Pictures and sentence meaning are valuable for comprehension and for confirming whether a decoded word makes sense. They should not replace attention to the letters when identifying an unfamiliar printed word. The safer sequence is **print first for word identity; context next for meaning and confirmation**. If a child repeatedly looks at the first letter, scans the picture and invents the rest of the word, bring attention back to the complete printed sequence.' },

    { type: 'h2', content: 'What systematic and cumulative phonics means' },
    { type: 'p', content: 'Systematic phonics follows a planned progression rather than introducing spellings randomly. Cumulative teaching keeps earlier correspondences active while new ones are added, so the child’s usable code grows without repeatedly losing the foundation. The [Education Endowment Foundation phonics review](https://educationendowmentfoundation.org.uk/education-evidence/teaching-learning-toolkit/phonics) supports explicit, systematic instruction, while the [IES foundational-reading practice guide](https://ies.ed.gov/ncee/wwc/PracticeGuide/21/Published) connects sound–letter teaching with decoding, word writing and connected-text reading.' },
    ...
    { type: 'h2', content: 'Where home support fits' },
    { type: 'p', content: 'At home, the safest starting point is to reinforce the sequence the child is actually being taught rather than introduce a second competing programme. Use a small amount of taught knowledge in retrieval, word reading, spelling and a fresh matched example, then notice where help is still needed.' },
    { type: 'p', content: 'This article does not own the full home-practice method. For the parent-support system, correction prompts, decodable books, games and home-reading routine, use [Phonics for Parents](/blog/phonics-for-parents-guide). For targeted practice only, Tiny Steps also provides [Phonics Games for Letter Sounds](/blog/phonics-games-for-letter-sounds), [Balloon Pop](/free-balloon-pop-phonics-game-for-kids) and the [Letter Tracing Game](/free-letter-tracing-game-for-kids).' },
    ...
    { type: 'li', content: '[NCERT — National Curriculum Framework for Foundational Stage 2022](https://www.ncert.nic.in/pdf/NCF_for_Foundational_Stage_20_October_2022.pdf): India’s foundational-stage framework places sound–letter connection inside a wider progression toward reading and writing words and connected text; its learning outcomes should be read as developmental trajectories rather than rigid age cut-offs.' },
  ...
  faq: [
    ...
    { question: 'What is the difference between phonemic awareness and phonics?', answer: 'Phonemic awareness is hearing and working with individual speech sounds without needing print. Phonics connects those speech sounds with written graphemes so children can use the relationship for reading and spelling.' },
