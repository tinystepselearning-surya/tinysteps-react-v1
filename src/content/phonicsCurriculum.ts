...
/**
 * Canonical Tiny Steps phonics curriculum approved on 2026-08-10.
 *
 * IMPORTANT:
 * - This is the authoritative lesson order for public curriculum and teacher progress UI.
 * - Lesson document ids deliberately remain `${courseId}__lesson-NN` so existing progress
 *   references are not orphaned when the curriculum copy/order is refreshed.
 * - Do not maintain a second phonics lesson list in feature components.
 */
...
export const PHONICS_COURSES: Record<PhonicsCourseId, PhonicsCourseDefinition> = {
  'phonics-foundations': {
    ...
    label: 'Foundation Phonics',
    publicSlug: 'phonics-foundation',
    lessonCount: FOUNDATION_LESSONS.length,
  ...
  'early-phonics': {
    ...
    lessonCount: EARLY_LESSONS.length,
  ...
  'advanced-phonics': {
    ...
    lessonCount: ADVANCED_LESSONS.length,
...
const getStageForLesson = (courseId: PhonicsCourseId, lessonNumber: number): PhonicsStage => {
  const stage = PHONICS_STAGE_DEFINITIONS[courseId].find(
    (candidate) => lessonNumber >= candidate.start && lessonNumber <= candidate.end,
  );
  if (!stage) {
    throw new Error(`Missing phonics stage for ${courseId} lesson ${lessonNumber}`);
  }
  return stage;
};
...
const buildLessons = (
  ...
  const course = PHONICS_COURSES[courseId];
  return labels.map((label, index) => {
    const lessonNumber = index + 1;
    const stage = getStageForLesson(courseId, lessonNumber);
...
export const PHONICS_LESSONS_BY_COURSE: Record<PhonicsCourseId, PhonicsLesson[]> = {
  'phonics-foundations': buildLessons('phonics-foundations', FOUNDATION_LESSONS),
  'early-phonics': buildLessons('early-phonics', EARLY_LESSONS),
  'advanced-phonics': buildLessons('advanced-phonics', ADVANCED_LESSONS),
};

export const PHONICS_CURRICULUM_TOPICS: PhonicsLesson[] = [
  ...PHONICS_LESSONS_BY_COURSE['phonics-foundations'],
  ...PHONICS_LESSONS_BY_COURSE['early-phonics'],
  ...PHONICS_LESSONS_BY_COURSE['advanced-phonics'],
];
