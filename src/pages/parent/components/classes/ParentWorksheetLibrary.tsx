    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-600">Practice resources shared for this child’s courses, organised lesson by lesson.</p>
        </div>
        <Button type="button" variant="outline" size="sm" onClick={onRefresh} disabled={refreshing} className="h-8 shrink-0">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div>
          <label htmlFor="parent-worksheet-search" className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Search</label>
          ...
          {Array.from(courseGroups.entries()).map(([courseKey, lessons]) => (
            <section key={courseKey} aria-label={lessons[0].courseTitle}>
              <div className="mb-2 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-950">{lessons[0].courseTitle}</h3>
                <span className="text-[11px] text-slate-500">{lessons.length} lesson{lessons.length === 1 ? '' : 's'}</span>
              </div>
                  ...
                  >
                    <div className={`relative h-24 overflow-hidden rounded-lg border border-white/80 bg-gradient-to-br ${WORKSHEET_TILE_THEMES[index % WORKSHEET_TILE_THEMES.length]}`}>
                      ...
                      <div className="absolute inset-x-2 top-2 flex items-center justify-between gap-2 text-[9px] font-semibold text-slate-600">
                        <span className="rounded-full bg-white/85 px-2 py-0.5">Worksheet</span>
                      ...
                      <div className="absolute inset-x-2 bottom-2 rounded-md bg-white/88 px-2 py-1.5 shadow-sm backdrop-blur-[1px]">
                        ...
                        <p className="mt-0.5 text-[10px] text-slate-500">
                          {lesson.items.length} practice resource{lesson.items.length === 1 ? '' : 's'}
                        </p>
                      </div>
                    </div>
                          ...
                          <div key={item.id} className="rounded-lg bg-slate-50 px-2 py-2">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <p className="truncate text-[12px] font-semibold text-slate-900">{item.title || 'Worksheet'}</p>
                                {item.resourceType ? (
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-1">
                              <Button type="button" size="sm" variant="outline" className="h-7 gap-1 px-1 text-[10px]" disabled={!openUrl} onClick={() => openUrl && openSafely(openUrl)}>
                                Open <ExternalLink className="h-3 w-3" aria-hidden="true" />
                              </Button>
                              {downloadUrl ? (
                                <Button type="button" size="sm" variant="outline" className="h-7 gap-1 px-1 text-[10px]" onClick={() => openSafely(downloadUrl)}>
                                  Download <Download className="h-3 w-3" aria-hidden="true" />
                                </Button>
                              ) : (
                                <Button type="button" size="sm" variant="outline" className="h-7 px-1 text-[10px]" disabled>Download</Button>
                              )}
                            </div>
                          </div>
                        );
