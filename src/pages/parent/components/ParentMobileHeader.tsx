...
export default function ParentMobileHeader({
  ...
  return (
    <header
      className={cn(
        "ts-parent-mobile-header sticky top-0 z-30 hidden shrink-0 border-b border-slate-200/80 bg-white/95 backdrop-blur-md max-lg:block",
        className,
      )}
    ...
    >
      <div className="flex h-14 min-w-0 items-center gap-2 px-2 sm:px-3">
        ...
        <div className="min-w-0 flex-1 px-1">
          <h1 className="truncate text-[15px] font-semibold leading-5 text-slate-950 sm:text-base">
            {getParentTabTitle(activeTab)}
          </h1>
          {normalizedChildName && (
            <p
              className="truncate text-xs leading-4 text-slate-600"
