...
const headerScript = { src: "/shared/header.js", type: "text/javascript", defer: true };
const BLOG_PAGES = {
    week1: {
...
export default function BlogArticle() {
    ...
    const config = hasConfig ? BLOG_PAGES[slug] : null;
    const transform = (doc) => {
    ...
    const { html, loading, error } = useLegacyPage(config
        ? {
            path: config.path,
    ...
    if (!hasConfig) {
        return _jsx(Navigate, { to: "/blog", replace: true });
    }
    if (loading) {
        return (_jsx("div", { className: "mx-auto max-w-6xl px-4 py-20 text-center text-gray-600", children: "Loading story\u2026" }));
    }
    if (error) {
        return (_jsx("div", { className: "mx-auto max-w-6xl px-4 py-20 text-center text-red-600", children: "We couldn\u2019t load this story. Please head back to the blog and try another article." }));
    }
