...
import {
  getPublishedPhonicsResourcePageByConceptId,
...
function learningLink(conceptId: string): LearningLink | null {
  ...
  if (concept.canonicalOwnerTopicId) return { id: conceptId, label: concept.label, to: getCanonicalTopicOwnerPath(concept.canonicalOwnerTopicId) };
...
const MiniLinkList: FC<{ title: string; items: readonly LearningLink[] }> = ({ title, items }) => {
  ...
  return <div><h3 className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">{title}</h3><div className="mt-3 flex flex-wrap gap-2">{items.map((item) => <Link key={`${title}-${item.id}-${item.to}`} to={item.to} className="rounded-full border border-slate-200 bg-white px-3.5 py-2 text-sm font-bold text-slate-700 transition hover:border-sky-300 hover:text-sky-800">{item.label} <span aria-hidden="true">→</span></Link>)}</div></div>;
};

const BulletPanel: FC<{ title: string; items: readonly string[]; tone?: 'plain' | 'warning' | 'practice' }> = ({ title, items, tone = 'plain' }) => {
  const toneClass = tone === 'warning' ? 'border-amber-200 bg-amber-50/70' : tone === 'practice' ? 'border-emerald-200 bg-emerald-50/60' : 'border-slate-200 bg-white';
  return <section className={`rounded-[1.6rem] border ${toneClass} p-5 sm:p-6`}><h2 className="text-xl font-black tracking-[-0.02em] text-slate-950">{title}</h2><ul className="mt-4 space-y-3 text-[15px] leading-7 text-slate-700">{items.map((item) => <li key={item} className="flex gap-3"><span aria-hidden="true" className="mt-[0.68rem] h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400" /><span>{item}</span></li>)}</ul></section>;
};
...
export default function PhonicsKnowledgePage() {
  ...
  const definedTermSchema = {
    '@context': 'https://schema.org', '@type': 'DefinedTerm', '@id': definedTermId,
    name: concept.label, description: concept.quickAnswer, url: canonicalUrl,
    inDefinedTermSet: { '@type': 'DefinedTermSet', '@id': `${SITE_ORIGIN}/resources/phonics#phonics-knowledge`, name: 'Tiny Steps Phonics & Reading Resources', url: `${SITE_ORIGIN}/resources/phonics` },
  };
  const webPageSchema = {
    '@context': 'https://schema.org', '@type': 'WebPage', '@id': `${canonicalUrl}#webpage`, url: canonicalUrl,
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_8%_4%,rgba(14,165,233,0.09),transparent_24%),radial-gradient(circle_at_92%_8%,rgba(251,146,60,0.08),transparent_22%),linear-gradient(180deg,#fbfaf7_0%,#ffffff_52%,#f8fafc_100%)] text-slate-950">
      <Meta title={page.seoTitle} description={page.seoDescription} canonical={canonicalUrl} robots="index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1" jsonLd={[organizationSchema, websiteSchema, webPageSchema, definedTermSchema, breadcrumbSchema]} />
      <article className="mx-auto max-w-5xl px-6 pb-16 pt-7 sm:pt-9 lg:pb-20">
        ...
        {concept.contrastWords.length ? <section className="mt-5 rounded-[1.6rem] border border-slate-200 bg-white p-5 sm:p-6"><h2 className="text-xl font-black tracking-[-0.02em] text-slate-950">Useful contrasts</h2><p className="mt-2 text-sm leading-6 text-slate-600">Use contrasts after the main pattern is secure. They help children see where a useful phonics generalisation stops applying.</p><div className="mt-4 flex flex-wrap gap-2">{concept.contrastWords.map((word) => <span key={word} className="rounded-full border border-slate-200 px-3 py-2 text-sm font-bold text-slate-700">{word}</span>)}</div></section> : null}

        <section className="mt-8 rounded-[1.8rem] border border-sky-100 bg-sky-50/45 p-5 sm:p-7" data-resource-related-guides>
          <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[0.2em] text-sky-700">Explore related phonics patterns</p><h2 className="mt-2 text-2xl font-black tracking-[-0.025em] text-slate-950">{discoveryCluster ? `More from ${discoveryCluster.label.toLowerCase()}` : 'Continue through the phonics resource graph'}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">These links are selected from the published Tiny Steps phonics graph, prioritising nearby patterns in the same family before adjacent curriculum concepts.</p></div><Link to="/resources/phonics" className="text-sm font-black text-sky-700 hover:text-sky-900">Browse all phonics resources →</Link></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{relatedGuides.map((related) => <Link key={related.path} to={related.path} data-resource-related-path={related.path} className="group rounded-2xl border border-sky-100 bg-white p-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)] transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-[0_12px_30px_rgba(15,23,42,0.07)]"><span className="block text-sm font-black leading-5 text-slate-950 group-hover:text-sky-800">{related.cardTitle}</span><span className="mt-1.5 line-clamp-2 block text-xs leading-5 text-slate-500">{related.concept.quickAnswer}</span></Link>)}</div>
        </section>
