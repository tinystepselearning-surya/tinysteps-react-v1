...
import BlogLeadAttributionSection from './BlogLeadAttributionSection';
import {
  addDaysToDateKey,
  dateKeyInRange,
  leadReceivedDateKey,
...
export default function LeadSourceAnalysisV5({
  startDateKey,
  endDateKey,
  showFunnel = true,
    ...
    try {
      const snapshots = await Promise.all(timestampFields.map((field, index) =>
        getDocsLogged(
          ...
          { source: 'src/pages/admin/LeadSourceAnalysisV5.tsx' },
        )
      ));
      ...
      if (requestSequenceRef.current !== requestSequence) return;
      console.error('[LeadSourceAnalysisV5] load failed', err);
    ...
    <div className="space-y-4">
      ...
      {showAttribution ? (
        <Card className="p-4 md:p-5">
          ...
          <BlogLeadAttributionSection leads={rows} loading={loading} error={error} />
