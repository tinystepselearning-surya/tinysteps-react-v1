...
const tense = (config) => freeze({
  ...
  childFriendlyMeaning: config.childFriendlyMeaning,
  ...
  teachingBoundary: config.teachingBoundary,
...
const errorPattern = (config) => freeze({
  id: config.id,
  category: config.category,
  tenseIds: freezeList(config.tenseIds),
  incorrectExample: config.incorrectExample,
  correction: config.correction,
  diagnosis: config.diagnosis,
  teachingResponse: config.teachingResponse,
});
...
/**
 * GR2 expands the single GR1 `tenses` node into a child-centred tense system.
 * This is a knowledge/practice architecture, not a request to publish one SEO
 * page per tense. Meanings and contrasts come before keyword matching, and time
 * clues remain teaching supports rather than mechanical tense-selection rules.
 */
...
export const GRAMMAR_WRITING_TENSE_NODES = freezeList([
  tense({
    ...
    meaningUses: ['habits and routines', 'repeated actions', 'facts and general truths', 'stable states in age-appropriate contexts'],
    ...
    prerequisiteTenseIds: [],
    nextTenseIds: ['simple-past', 'present-continuous'],
  ...
  tense({
    ...
    family: 'simple',
    timeFrame: 'past',
    childFriendlyMeaning: 'Use the simple past for an event, action or state understood as completed in the past, including both regular -ed forms and high-frequency irregular verbs.',
    ...
    meaningUses: ['completed past events', 'past actions in a sequence', 'finished past states', 'retelling what happened'],
    ...
    prerequisiteTenseIds: ['simple-present'],
    nextTenseIds: ['simple-future-will', 'past-continuous', 'present-perfect'],
    comparisonIds: ['simple-past-vs-past-continuous', 'present-perfect-vs-simple-past', 'past-perfect-vs-simple-past'],
    commonErrorIds: ['past-base-form-in-past-context', 'irregular-past-overgeneralisation', 'did-double-past-marking'],
  ...
  tense({
    ...
    meaningUses: ['predictions', 'decisions made at the moment', 'promises and offers', 'straightforward future statements for beginners'],
    ...
    prerequisiteTenseIds: ['simple-past'],
    nextTenseIds: ['future-forms'],
    comparisonIds: ['future-forms-choice'],
    commonErrorIds: ['will-nonbase-verb'],
  ...
  tense({
    ...
    meaningUses: ['actions happening now', 'temporary actions around the present', 'changing or developing situations at an age-appropriate level', 'arranged future events in later instruction'],
    ...
    prerequisiteTenseIds: ['simple-present'],
    nextTenseIds: ['past-continuous', 'future-forms'],
    comparisonIds: ['simple-present-vs-present-continuous'],
  ...
  tense({
    ...
    meaningUses: ['actions in progress at a past time', 'background actions in a story', 'an ongoing action interrupted by another event', 'two past actions happening at the same time'],
    ...
    prerequisiteTenseIds: ['simple-past', 'present-continuous'],
    nextTenseIds: ['tense-consistency-transfer'],
  ...
  tense({
    id: 'future-forms',
    order: 6,
    label: 'Future Forms & Choice',
    family: 'future-system',
    timeFrame: 'future',
    childFriendlyMeaning: 'Choose among common future constructions according to meaning: will for appropriate predictions or decisions, be going to for intentions or evidence-based predictions, and present forms for arrangements or schedules where suitable.',
    ...
    prerequisiteTenseIds: ['simple-future-will', 'present-continuous'],
    nextTenseIds: ['tense-consistency-transfer'],
    comparisonIds: ['future-forms-choice'],
    commonErrorIds: ['going-to-missing-be', 'future-form-meaning-mismatch', 'will-nonbase-verb'],
    writingApplications: ['write plans and intentions', 'choose a natural future form for predictions or arrangements', 'edit repetitive overuse of will'],
    curriculumAnchors: [
      { courseSlug: 'advanced-grammar', lessonNumber: 9, lessonTitle: 'Talking About the Future Naturally' },
    ],
  }),
  tense({
    ...
    meaningUses: ['life experience without a finished past time', 'recent events with present relevance', 'unfinished time periods', 'situations continuing to the present in age-appropriate contexts'],
    ...
    prerequisiteTenseIds: ['simple-past'],
    nextTenseIds: ['past-perfect', 'tense-consistency-transfer'],
    ...
    writingApplications: ['write about experiences', 'connect a recent event to a present result', 'choose between finished past and present-linked past'],
    curriculumAnchors: [
      { courseSlug: 'advanced-grammar', lessonNumber: 10, lessonTitle: 'Present Perfect & Simple Past' },
    ],
  }),
  tense({
    ...
    meaningUses: ['earlier event before another past event', 'clarifying event sequence', 'showing a completed earlier-past background when needed'],
    ...
    prerequisiteTenseIds: ['simple-past', 'present-perfect'],
    nextTenseIds: ['tense-consistency-transfer'],
  ...
  tense({
    id: 'tense-consistency-transfer',
    order: 9,
    label: 'Tense Consistency & Transfer',
    family: 'transfer',
    ...
    prerequisiteTenseIds: ['past-continuous', 'future-forms', 'present-perfect', 'past-perfect'],
    nextTenseIds: [],
    comparisonIds: [],
    commonErrorIds: ['tense-drift-connected-language', 'clue-word-matching-without-meaning', 'controlled-practice-transfer-gap'],
    writingApplications: ['maintain tense in a paragraph', 'edit narrative tense drift', 'shift time intentionally in explanation or storytelling', 'transfer tense control from exercises into fresh writing'],
    curriculumAnchors: [
      { courseSlug: 'advanced-grammar', lessonNumber: 12, lessonTitle: 'Tense Mastery: Speak, Write & Edit' },
    ],
  }),
]);
...
export const GRAMMAR_WRITING_TENSE_COMPARISONS = freezeList([
  comparison({
    id: 'simple-present-vs-present-continuous',
    order: 1,
    label: 'Simple Present vs Present Continuous',
    tenseIds: ['simple-present', 'present-continuous'],
    decisionQuestion: 'Is this a routine/general truth, or an action/temporary situation in progress around now?',
    distinction: 'Use simple present for routines, repeated actions and general states; use present continuous for actions or temporary situations in progress around the present.',
    contrastExamples: ['Maya wears glasses. / Maya is wearing sunglasses today.', 'I read every evening. / I am reading now.'],
    commonErrorIds: ['simple-present-now-confusion', 'continuous-missing-be'],
  ...
  comparison({
    id: 'simple-past-vs-past-continuous',
    order: 2,
    label: 'Simple Past vs Past Continuous',
    tenseIds: ['simple-past', 'past-continuous'],
    decisionQuestion: 'Is the past event viewed as completed, or was it in progress at a particular past moment/background?',
    distinction: 'Use simple past for completed events and event steps; use past continuous for an ongoing past frame, often around another event.',
    contrastExamples: ['The phone rang. / I was reading when the phone rang.', 'We played outside. / We were playing outside when it started to rain.'],
    commonErrorIds: ['past-base-form-in-past-context', 'past-continuous-wrong-auxiliary'],
  ...
  comparison({
    ...
    order: 3,
    label: 'Choosing Future Forms',
    tenseIds: ['simple-future-will', 'future-forms'],
    decisionQuestion: 'Is the speaker predicting, deciding, expressing an intention, describing an arrangement, or referring to a fixed schedule?',
    distinction: 'Future meaning can use will, be going to, present continuous or simple present depending on purpose and context; the forms are related but not interchangeable.',
    contrastExamples: ['I think it will rain. / Look at those clouds—it is going to rain.', 'I will call her. / I am meeting her at five.'],
    commonErrorIds: ['future-form-meaning-mismatch', 'going-to-missing-be', 'will-nonbase-verb'],
    curriculumAnchors: [{ courseSlug: 'advanced-grammar', lessonNumber: 9, lessonTitle: 'Talking About the Future Naturally' }],
  }),
  comparison({
    id: 'present-perfect-vs-simple-past',
    order: 4,
    label: 'Present Perfect vs Simple Past',
    tenseIds: ['present-perfect', 'simple-past'],
    decisionQuestion: 'Is the event tied to a finished past time, or is the focus an experience/result connected to the present without a finished past time point?',
    distinction: 'Use simple past with completed past-time reference; use present perfect when the past is connected to the present and the finished past time is not the focus.',
    contrastExamples: ['I visited Delhi last year. / I have visited Delhi twice.', 'She finished her homework at six. / She has just finished her homework.'],
    commonErrorIds: ['present-perfect-finished-time-marker', 'present-perfect-wrong-auxiliary-or-participle'],
    curriculumAnchors: [{ courseSlug: 'advanced-grammar', lessonNumber: 10, lessonTitle: 'Present Perfect & Simple Past' }],
  }),
  comparison({
    id: 'past-perfect-vs-simple-past',
    order: 5,
    label: 'Past Perfect & Event Sequence',
    tenseIds: ['past-perfect', 'simple-past'],
    decisionQuestion: 'Do we need to make one past event explicitly earlier than another past reference point?',
    distinction: 'Simple past can narrate a clear chronological sequence; past perfect is useful when an earlier-past event must be distinguished from another past event.',
    contrastExamples: ['We arrived and the film started. / The film had started before we arrived.', 'She finished the work and called me. / She had finished the work before she called me.'],
    commonErrorIds: ['past-perfect-sequence-confusion'],
...
export const GRAMMAR_WRITING_TENSE_ERROR_PATTERNS = freezeList([
  errorPattern({ id: 'simple-present-now-confusion', category: 'meaning', tenseIds: ['simple-present', 'present-continuous'], incorrectExample: 'I play football now.', correction: 'I am playing football now.', diagnosis: 'The child may be using “present” as a label for all current-time meaning rather than distinguishing routine/general meaning from an action in progress.', teachingResponse: 'Contrast the same verb in a routine sentence and a right-now sentence, then remove the clue word and ask the child to explain the meaning choice.' }),
  errorPattern({ id: 'third-person-s-omission', category: 'form', tenseIds: ['simple-present'], incorrectExample: 'She walk to school every day.', correction: 'She walks to school every day.', diagnosis: 'The child has the correct time meaning but has not consistently marked third-person singular present agreement.', teachingResponse: 'Keep the time meaning stable and practise subject–verb contrasts such as I walk / she walks before returning to fresh sentences.' }),
  errorPattern({ id: 'continuous-missing-be', category: 'form', tenseIds: ['present-continuous'], incorrectExample: 'She running now.', correction: 'She is running now.', diagnosis: 'The child recognises the -ing idea but is treating the participle as a complete verb phrase without am/is/are.', teachingResponse: 'Build the continuous as a two-part verb phrase: correct form of be + verb-ing, then vary the subject so the auxiliary must change.' }),
  errorPattern({ id: 'past-base-form-in-past-context', category: 'form', tenseIds: ['simple-past'], incorrectExample: 'Yesterday I go to school.', correction: 'Yesterday I went to school.', diagnosis: 'The child understands past time but defaults to a familiar base verb or has not secured the required irregular form.', teachingResponse: 'Acknowledge the correct time meaning, supply or retrieve the past form, then reuse that verb in another fresh past sentence.' }),
  errorPattern({ id: 'irregular-past-overgeneralisation', category: 'morphology', tenseIds: ['simple-past'], incorrectExample: 'Yesterday I eated pizza.', correction: 'Yesterday I ate pizza.', diagnosis: 'The child has discovered the regular -ed pattern and over-applied it to an irregular verb.', teachingResponse: 'Preserve the successful past-time idea, contrast regular and irregular forms, and recycle a small high-frequency irregular set in meaningful sentences.' }),
  ...
  errorPattern({ id: 'past-continuous-wrong-auxiliary', category: 'form', tenseIds: ['past-continuous'], incorrectExample: 'They was playing outside.', correction: 'They were playing outside.', diagnosis: 'The child understands ongoing past meaning but has not matched was/were to the subject.', teachingResponse: 'Briefly reconnect the form to subject agreement, then practise the whole was/were + verb-ing phrase inside background-event sentences.' }),
  errorPattern({ id: 'will-nonbase-verb', category: 'form', tenseIds: ['simple-future-will', 'future-forms'], incorrectExample: 'I will went tomorrow.', correction: 'I will go tomorrow.', diagnosis: 'The child is marking future with will but is also changing the main verb instead of keeping the base form.', teachingResponse: 'Treat will as the time-carrying auxiliary and contrast will go / went / am going with the same lexical verb.' }),
  errorPattern({ id: 'going-to-missing-be', category: 'form', tenseIds: ['future-forms'], incorrectExample: 'We going to visit Grandma.', correction: 'We are going to visit Grandma.', diagnosis: 'The child knows the going to sequence but omits the required form of be.', teachingResponse: 'Build the construction as subject + am/is/are + going to + base verb and vary subjects before returning to plan/intent examples.' }),
  errorPattern({ id: 'future-form-meaning-mismatch', category: 'meaning', tenseIds: ['future-forms'], incorrectExample: 'I will meet the dentist at 4:00; the appointment is already arranged.', correction: 'I am meeting the dentist at 4:00.', diagnosis: 'The sentence may be grammatically possible, but the child is not yet choosing a future form that naturally matches the intended arrangement/plan meaning.', teachingResponse: 'Give two or three future contexts and ask what kind of meaning each expresses—prediction, decision, intention, arrangement or schedule—before selecting the form.' }),
  errorPattern({ id: 'present-perfect-wrong-auxiliary-or-participle', category: 'form', tenseIds: ['present-perfect'], incorrectExample: 'She has went home.', correction: 'She has gone home.', diagnosis: 'The child is using the correct perfect auxiliary but substitutes a simple-past form for the past participle.', teachingResponse: 'Practise a small verb family across base / past / past participle, then rebuild the full have/has + participle pattern in context.' }),
  errorPattern({ id: 'present-perfect-finished-time-marker', category: 'meaning', tenseIds: ['present-perfect', 'simple-past'], incorrectExample: 'I have visited Delhi last year.', correction: 'I visited Delhi last year.', diagnosis: 'The child is combining present perfect with an explicitly finished past-time reference where simple past is normally required.', teachingResponse: 'Contrast a finished-time sentence with an experience sentence: last year → simple past; no finished time / life experience → present perfect.' }),
  errorPattern({ id: 'past-perfect-sequence-confusion', category: 'meaning', tenseIds: ['past-perfect', 'simple-past'], incorrectExample: 'I had went to school before the rain started.', correction: 'I had gone to school before the rain started.', diagnosis: 'The child may understand the earlier-past relationship but has not formed the participle correctly, or may use past perfect mechanically whenever before/after appears.', teachingResponse: 'First establish the two past events and their order, then decide whether earlier-past marking is useful; only after the meaning is clear focus on had + participle.' }),
  errorPattern({ id: 'tense-drift-connected-language', category: 'consistency', tenseIds: ['tense-consistency-transfer'], incorrectExample: 'Yesterday we went to the park. We play football and saw a dog.', correction: 'Yesterday we went to the park. We played football and saw a dog.', diagnosis: 'The child can form individual tense examples but loses the chosen time frame while producing connected language.', teachingResponse: 'Use a short retell or paragraph, mark the intended time frame once, and ask the child to audit each verb for purposeful consistency rather than correcting isolated blanks.' }),
  errorPattern({ id: 'clue-word-matching-without-meaning', category: 'strategy', tenseIds: ['tense-consistency-transfer'], incorrectExample: 'The child always chooses a memorised tense whenever one clue word appears, even when the wider sentence meaning requires another form.', correction: 'Use the full sentence meaning and context to choose the tense; clue words support the decision but do not make it automatically.', diagnosis: 'The learner may have learned a keyword-matching test strategy instead of a transferable time-and-meaning system.', teachingResponse: 'Vary or remove clue words, use paired contexts with the same clue where possible, and require a short explanation of why the chosen tense fits the meaning.' }),
  errorPattern({ id: 'controlled-practice-transfer-gap', category: 'transfer', tenseIds: ['tense-consistency-transfer'], incorrectExample: 'The child scores well on tense worksheets but switches forms unpredictably in spontaneous speaking or independent writing.', correction: 'Move from recognition and controlled choice into oral production, fresh sentence generation, connected writing and self-editing.', diagnosis: 'Tense knowledge is available under prompts but is not yet automatic or independently retrievable during real language production.', teachingResponse: 'Use the Tiny Steps progression HEAR → NOTICE → CHOOSE → SAY → WRITE → TRANSFER, reducing prompts and changing topics so the child must retrieve the tense independently.' }),
]);
...
export const GRAMMAR_WRITING_TENSE_NEXT_EDGES = freezeList(
  GRAMMAR_WRITING_TENSE_NODES.flatMap((source) => source.nextTenseIds.map((targetTenseId) => freeze({ sourceTenseId: source.id, relation: 'next', targetTenseId }))),
);
...
function canReach(startId, targetId) {
  ...
  while (queue.length) {
    ...
    seen.add(currentId);
    const current = tenseById.get(currentId);
    for (const nextId of current?.nextTenseIds ?? []) queue.push(nextId);
  }
  return false;
}
...
if (tenseById.size !== GRAMMAR_WRITING_TENSE_NODES.length) throw new Error('GR2 contains duplicate tense IDs.');
if (comparisonById.size !== GRAMMAR_WRITING_TENSE_COMPARISONS.length) throw new Error('GR2 contains duplicate comparison IDs.');
if (errorById.size !== GRAMMAR_WRITING_TENSE_ERROR_PATTERNS.length) throw new Error('GR2 contains duplicate error-pattern IDs.');
if (GRAMMAR_WRITING_TENSE_NODES.map((item) => item.order).join(',') !== '1,2,3,4,5,6,7,8,9') throw new Error('GR2 tense order must remain explicit and stable from 1 through 9.');
if (GRAMMAR_WRITING_TENSE_COMPARISONS.map((item) => item.order).join(',') !== '1,2,3,4,5') throw new Error('GR2 comparison order must remain explicit and stable from 1 through 5.');

for (const item of GRAMMAR_WRITING_TENSE_NODES) {
  if (!item.childFriendlyMeaning || item.childFriendlyMeaning.length < 80) throw new Error(`GR2 tense meaning is too thin: ${item.id}.`);
  if (!item.meaningUses.length || !item.examples.length || !item.teachingBoundary || !item.writingApplications.length || !item.curriculumAnchors.length) throw new Error(`GR2 tense node is incomplete: ${item.id}.`);
  ...
  for (const prerequisiteId of item.prerequisiteTenseIds) {
    if (!tenseById.has(prerequisiteId)) throw new Error(`GR2 unknown prerequisite: ${item.id} -> ${prerequisiteId}.`);
    if (!tenseById.get(prerequisiteId).nextTenseIds.includes(item.id)) throw new Error(`GR2 prerequisite edge is not reciprocal: ${prerequisiteId} -> ${item.id}.`);
  }
  for (const nextId of item.nextTenseIds) {
    if (!tenseById.has(nextId)) throw new Error(`GR2 unknown next tense: ${item.id} -> ${nextId}.`);
    if (!tenseById.get(nextId).prerequisiteTenseIds.includes(item.id)) throw new Error(`GR2 next edge is not reciprocal: ${item.id} -> ${nextId}.`);
  }
  for (const comparisonId of item.comparisonIds) {
    const entry = comparisonById.get(comparisonId);
    if (!entry || !entry.tenseIds.includes(item.id)) throw new Error(`GR2 comparison is not reciprocal: ${item.id} -> ${comparisonId}.`);
  }
  for (const errorId of item.commonErrorIds) {
    const entry = errorById.get(errorId);
    if (!entry || !entry.tenseIds.includes(item.id)) throw new Error(`GR2 error pattern is not reciprocal: ${item.id} -> ${errorId}.`);
  }
}

for (const item of GRAMMAR_WRITING_TENSE_COMPARISONS) {
  if (item.tenseIds.length < 2) throw new Error(`GR2 comparison must connect at least two tense nodes: ${item.id}.`);
  for (const tenseId of item.tenseIds) {
    if (!tenseById.has(tenseId)) throw new Error(`GR2 comparison points to unknown tense: ${item.id} -> ${tenseId}.`);
    if (!tenseById.get(tenseId).comparisonIds.includes(item.id)) throw new Error(`GR2 comparison backlink is missing: ${item.id} -> ${tenseId}.`);
  }
...
for (const item of GRAMMAR_WRITING_TENSE_ERROR_PATTERNS) {
  ...
  for (const tenseId of item.tenseIds) {
    if (!tenseById.has(tenseId)) throw new Error(`GR2 error points to unknown tense: ${item.id} -> ${tenseId}.`);
    if (!tenseById.get(tenseId).commonErrorIds.includes(item.id) && item.category !== 'meaning') throw new Error(`GR2 error backlink is missing: ${item.id} -> ${tenseId}.`);
  }
}
...
for (const item of GRAMMAR_WRITING_TENSE_NODES) {
  if (!canReach(GRAMMAR_WRITING_TENSE_ROOT_ID, item.id)) throw new Error(`GR2 tense is unreachable from the root: ${item.id}.`);
