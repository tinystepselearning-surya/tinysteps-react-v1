...
export function captureLeadAttribution(): LeadAttributionCapture {
  ...
  const next: LeadAttributionCapture = compactObject({
    ...
    referrer: existing ? existing.referrer : externalReferrer.referrer,
    referrerDomain: existing ? existing.referrerDomain : externalReferrer.referrerDomain,
    utmSource: existing ? existing.utmSource : sanitizeValue(params.get('utm_source'), 120),
    utmMedium: existing ? existing.utmMedium : sanitizeValue(params.get('utm_medium'), 120),
    utmCampaign: existing ? existing.utmCampaign : sanitizeValue(params.get('utm_campaign'), 160),
    utmTerm: existing ? existing.utmTerm : sanitizeValue(params.get('utm_term'), 160),
    utmContent: existing ? existing.utmContent : sanitizeValue(params.get('utm_content'), 160),
    gclid: existing ? existing.gclid : sanitizeValue(params.get('gclid'), 160),
    fbclid: existing ? existing.fbclid : sanitizeValue(params.get('fbclid'), 160),
    msclkid: existing ? existing.msclkid : sanitizeValue(params.get('msclkid'), 160),
