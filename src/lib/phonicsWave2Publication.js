...
const approvedIds = Object.keys(WAVE_2_APPROVALS);
if (approvedIds.length !== PHONICS_WAVE_2_PAGE_COUNT || wave2ConceptIds.size !== PHONICS_WAVE_2_PAGE_COUNT) {
  throw new Error(`R12 Wave 2 scope drift: ${approvedIds.length} approvals for ${wave2ConceptIds.size} future-wave-2 concepts.`);
}
...
for (const conceptId of wave2ConceptIds) if (!WAVE_2_APPROVALS[conceptId]) throw new Error(`R12 requires explicit approval for: ${conceptId}`);

function buildPage(conceptId, approval) {
  const concept = getPhonicsKnowledgeConcept(conceptId);
  if (!concept || concept.expansionState !== 'future-wave-2' || !concept.futureSlugCandidate) throw new Error(`Invalid R12 Wave 2 concept: ${conceptId}`);
  if (concept.canonicalOwnerTopicId) throw new Error(`R12 refuses an already-owned concept: ${conceptId}`);
...
export const PHONICS_WAVE_2_PAGES = freezeList(Object.entries(WAVE_2_APPROVALS).map(([id, approval]) => buildPage(id, approval)).sort((a, b) => a.concept.progressionRank - b.concept.progressionRank || a.cardTitle.localeCompare(b.cardTitle)));
