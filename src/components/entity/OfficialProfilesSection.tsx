      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-center lg:gap-12">
          <div>
            ...
            >
              Find {PUBLIC_FACTS.brandName} online
            </h2>
        ...
        <div className="mt-10 border-t border-slate-200 pt-8">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Founder identity</p>
          <div className="mt-4 grid gap-3 sm:max-w-xl">
            {FOUNDER_PUBLIC_PROFILES.map((profile) => (
              <a
                key={profile.platform}
                href={profile.url}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex min-h-20 items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm transition hover:border-orange-300 hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-orange-600"
                aria-label={`${PUBLIC_FACTS.founder.fullName} on ${profile.platform} (opens in a new tab)`}
              >
                <span>
                  <span className="block text-sm font-bold text-slate-950">
                    {PUBLIC_FACTS.founder.fullName} on {profile.platform}
                  </span>
                  <span className="mt-1 block text-xs leading-5 text-slate-500">{profile.purpose}</span>
                </span>
                <ExternalLink
