    ...
    <section data-animate="fade-up" className="bg-gradient-to-b from-white to-slate-50/50 py-16">
      <div className="mx-auto max-w-6xl px-6">
        <div className="text-center">
          <div className="gradient-chip mx-auto w-max">Purposeful screen time</div>
          <h2 className="mt-2 text-3xl font-semibold text-gray-900 md:text-4xl">{heading}</h2>
          <p className="mt-2 text-gray-700">Swap aimless screentime for curated Tiny Steps games. Kids play, parents get insight summaries.</p>
        </div>
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {plans.map((plan) => (
            <div key={plan.name} className={`rounded-3xl border border-white/0 bg-gradient-to-br ${plan.tone} p-[1px] shadow-card-hover`}>
              <div className="rounded-3xl bg-white/95 p-6">
                <div className="text-sm font-semibold uppercase tracking-wide text-gray-500">{plan.name}</div>
        ...
        <div className={`mt-8 rounded-[32px] border border-white/0 bg-gradient-to-r ${bundle.tone} p-[1px] shadow-card-hover`}>
          <div className="rounded-[28px] bg-white/95 p-6 md:p-8 grid gap-6 md:grid-cols-[1.2fr_0.8fr] items-center">
            <div>
              <div className="text-sm font-semibold uppercase tracking-wide text-gray-500">Bundle & save</div>
              <h3 className="text-2xl font-semibold text-gray-900">{bundle.name}</h3>
              <div className="text-4xl font-bold text-gray-900">{bundle.price}</div>
              ...
              <p className="mt-2 text-xs text-gray-500">Annual billing? Take an extra 10% off on each game plan.</p>
            </div>
