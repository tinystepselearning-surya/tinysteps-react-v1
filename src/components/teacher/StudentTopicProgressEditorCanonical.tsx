...
export default function StudentTopicProgressEditorCanonical({
  kidId,
  kidName,
  enrollmentId,
  onSaveAndBack,
  ...
  useEffect(() => {
    ...
    const nextStrengths = Array.isArray((existing as any)?.strengthSubskills)
      ? (existing as any).strengthSubskills.filter((item: unknown) => typeof item === 'string' && allowedLabels.has(item as string)).slice(0, 3)
      : [];
    const nextPractice = Array.isArray((existing as any)?.needsPracticeSubskills)
      ? (existing as any).needsPracticeSubskills.filter((item: unknown) => typeof item === 'string' && allowedLabels.has(item as string)).slice(0, 3)
      : [];
    ...
    <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="text-xs text-slate-500">Select the lesson taught today and update the child&apos;s progress.</div>
        </div>
        {saveMessage ? (
          <span className={saveMessage === 'Progress saved.' ? 'text-xs text-emerald-600' : 'text-xs text-red-600'}>
            {saveMessage}
          </span>
        ) : null}
      </div>
            ...
            {chipLabels.map((label) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-xs font-medium ${active ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-emerald-200 bg-white text-emerald-900'}`}
                >
                  {label}
                </button>
              );
            ...
            {chipLabels.map((label) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 text-xs font-medium ${active ? 'border-amber-600 bg-amber-600 text-white' : 'border-amber-200 bg-white text-amber-900'}`}
                >
                  {label}
                </button>
              );
