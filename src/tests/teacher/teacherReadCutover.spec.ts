import fs from 'node:fs';
import { describe, expect, it } from 'vitest';

const CANONICAL_ONLY_TEACHER_READERS = [
  'src/pages/teacher/hooks/useTeacherSessions.ts',
  'src/pages/teacher/hooks/useUpcomingSessions.ts',
  'src/pages/teacher/components/students/TeacherMyStudentsV2.tsx',
  'src/hooks/useTeacherFilteredData.ts',
] as const;

const ALL_ACTIVE_TEACHER_READERS = [
  ...CANONICAL_ONLY_TEACHER_READERS,
  'src/pages/teacher/hooks/useTeacherStudents.ts',
] as const;

const SCALAR_LEGACY_ALIAS_QUERY_PATTERNS = [
  /where\(\s*['\"]assignedTeacherId['\"]/, 
  /where\(\s*['\"]primaryTeacherId['\"]/, 
  /where\(\s*['\"]teacherUid['\"]/, 
  /where\(\s*['\"]teacher_id['\"]/, 
] as const;

describe('B4 canonical teacher collection reads', () => {
  it.each(ALL_ACTIVE_TEACHER_READERS)('%s does not call the legacy alias collection fallback helper', (path) => {
    const source = fs.readFileSync(path, 'utf8');
    expect(source).not.toContain('fetchTeacherSessionAliasFallbacks');
  });

  it.each(CANONICAL_ONLY_TEACHER_READERS)('%s does not query operational legacy teacher aliases', (path) => {
    const source = fs.readFileSync(path, 'utf8');
    expect(source).not.toMatch(/where\(\s*['\"]teacherIds['\"]/);
    SCALAR_LEGACY_ALIAS_QUERY_PATTERNS.forEach((pattern) => expect(source).not.toMatch(pattern));
  });

  it('keeps enrollment reads canonical in useTeacherStudents while preserving kids.teacherIds', () => {
    const source = fs.readFileSync('src/pages/teacher/hooks/useTeacherStudents.ts', 'utf8');

    SCALAR_LEGACY_ALIAS_QUERY_PATTERNS.forEach((pattern) => expect(source).not.toMatch(pattern));
    expect(source).toContain("query(enrollmentsBase, where('teacherId', '==', teacherId))");
    expect(source).toContain("query(collection(db, 'kids'), where('teacherIds', 'array-contains', teacherId))");
    expect(source).toContain('kids-by-teacherIds-fallback');
  });
});
