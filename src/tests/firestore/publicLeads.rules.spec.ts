...
function buildPublicLeadPayload() {
  return {
    parentName: 'Priya',
...
suite('public leads firestore rules', () => {
  it('allows unauthenticated create on the public leads collection', async () => {
    const db = testEnv.unauthenticatedContext().firestore();
    await assertSucceeds(addDoc(collection(db, 'leads'), buildPublicLeadPayload()));
  });
