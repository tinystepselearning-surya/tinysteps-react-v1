...
describe('SEO recovery Brick 15 stabilization', () => {
  ...
  it('locks the three stabilization phases and prevents early ranking conclusions', () => {
    expect(SEO_RECOVERY_BRICK15_PHASES.map((entry) => entry.id)).toEqual([
      'days-1-7',
      'days-14-21',
      'day-28-plus',
    ]);
    ...
    expect(deployment.checks).toEqual(expect.arrayContaining([
      'redirects',
      'canonical tags',
      'sitemap',
      'indexing',
  ...
  it('requires query-owner and commercial search review at days 14–21', () => {
    ...
    expect(preliminary.checks).toEqual(expect.arrayContaining([
  ...
  it('inherits the locked GSC baseline without pretending it is post-deployment evidence', () => {
    expect(SEO_RECOVERY_BRICK15_BASELINE.current).toMatchObject({
  ...
  it('inherits the six Brick 14 owner targets and residual watches', () => {
    ...
    expect(SEO_RECOVERY_BRICK15_QUERY_OWNER_TARGETS).toContainEqual(expect.objectContaining({
    ...
    expect(SEO_RECOVERY_BRICK15_RESIDUAL_WATCHLIST).toContainEqual(expect.objectContaining({
      query: 'best phonics classes online',
      severity: 'high',
    }));
  });
  ...
  it('reuses the technical consolidation audit while reserving live Google checks for observation', () => {
    ...
    expect(SEO_RECOVERY_BRICK15_TECHNICAL_AUTOMATION.liveChecksStillRequired).toEqual(expect.arrayContaining([
