...
describe('Resources architecture R12 controlled programmatic expansion', () => {
  ...
  it('requires every published page to pass the pre-publication quality gate before entering the registry', () => {
    for (const page of PHONICS_PUBLISHED_RESOURCE_PAGES) {
      expect(page.prepublicationQualityState).toBe(PHONICS_PREPUBLICATION_QUALITY_STATE);
      expect(page.prepublicationQualityRevision).toBe(PHONICS_PREPUBLICATION_QUALITY_REVISION);
      expect(page.prepublicationQualityChecks.length).toBeGreaterThanOrEqual(10);
    }
  });
  ...
  it('uses one governed reusable renderer and returns not-found for arbitrary slugs', () => {
    ...
    expect(source).not.toContain('usePublicEditorialApproval');
    expect(source).not.toContain('Reviewed for phonics accuracy by');
