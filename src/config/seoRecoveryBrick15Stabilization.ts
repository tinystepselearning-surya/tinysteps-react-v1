...
/**
 * Brick 15 is a stabilization contract, not an instant ranking verdict.
 * The implementation date records when the control plane was created. A live
 * deployment timestamp must be verified separately before any observation-day
 * clock is treated as having started.
 */
...
export const SEO_RECOVERY_BRICK15_TECHNICAL_AUTOMATION = Object.freeze({
  ...
  validates: Object.freeze([
    'self-canonical priority routes',
    ...
    'canonical internal-link rewrite controls',
    'build and prebuild SEO ordering',
  ]),
  liveChecksStillRequired: Object.freeze([
    'live redirect responses',
    'rendered canonical tags on production',
    'Google-selected canonical',
    'live indexing state',
  ]),
});
...
export const SEO_RECOVERY_BRICK15_DECISION_RULES = Object.freeze({
  ...
  finalClosureRule:
    'Close the recovery activity only after the 28+ day finalized comparison, technical checks, query-owner review and business-context review are complete with no unresolved material cannibalisation.',
});
