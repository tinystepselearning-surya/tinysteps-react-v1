...
import PhaseGridSkeleton from "../../components/phases/PhaseGridSkeleton";
...
import GamesGalleryPage from "./GamesGalleryPage";
...
export default function GamesGallery() {
  ...
  // Simulate initial load (in real app, this would be data fetching)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-sky-50 to-rose-50">
      <div className="mx-auto max-w-7xl px-4 pb-24 md:px-6">
        {/* Hero strip - slim header with title + Parent View toggle */}
        <div className="flex flex-col gap-4 border-b border-orange-100/50 py-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-gray-900 md:text-3xl">
              {parentView ? "📊 Learning Dashboard" : "🎮 Phonics Journey"}
            </h1>
            <p className="mt-1 text-sm text-gray-500 leading-relaxed">
              {parentView 
                ? "Track progress, view achievements, and download reports" 
        ...
        <div className="mt-6 animate-fadeIn" role="tabpanel" id={`panel-${viewMode}`}>
          {isLoading && viewMode === "cards" ? (
            <PhaseGridSkeleton count={visible.length || 8} />
          ) : (
            <>
              {viewMode === "cards" && (
              ...
              {viewMode === "games" && (
                <GameCatalogProvider>
                  <GamesGalleryPage embedded={true} />
                </GameCatalogProvider>
              )}
            </>
          )}
